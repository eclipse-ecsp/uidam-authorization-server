...
class ConfigRefreshListenerTest {
    ...
    static Stream<Arguments> providePropertyChangeArgs() {
        return Stream.of(
            Arguments.of("tenant.multitenant.enabled", (String) null),
    ...
    @ParameterizedTest
    @MethodSource("providePropertyChangeArgs")
    void onApplicationEvent_withVariousPropertyChanges_doesNotThrow(String key, String value) {
        Set<String> changedKeys = new HashSet<>();
        changedKeys.add(key);
        when(event.getKeys()).thenReturn(changedKeys);
